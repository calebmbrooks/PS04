...
Both curves show a dramatic increase in accuracy when both income and limit are used as predictors, but strangely the RMSE values begin to increase for test-data predictions from subsequent models, whereas on training data they continue to decrease. This could indicate an interaction between the predictors (introduced AFTER credit cart limit, particularly Age and Education given that the increase in rmse for those models is more visualy dramatic) which is accounted for when predicting from the training data, but not when predicting out-of-sample
